...
def localSearchHeuristic(G, pro, limits):
    ...
    if not fulfillsStrictRequirements(G, pro, start) or not fulfillsStrictRequirements(
        G, pro, end
    ):
        print(
            "Either start or end did not fulfill the strict requirements, so no path could be found."
        )
        return 0, 0, 0
    ...
    newPath, totalBER, improvement = augmentPathToBiggestSubset(
        G, pro, path, depthLimit, neighbourLimit
    )
        ...
        for i in range(len(originalPath) - detourDistance):
            if originalPath[i] in path and originalPath[i + detourDistance] in path:
                ...
                for c in set(path).difference(set(bottleneck)):
                    bottleneckFreeBER = bottleneckFreeBER.intersection(
                        G.nodes[c]["features"]
                    )
                ...
                if updatePath:
                    potential_path = (
                        path[: startIndex + 1] + bestDetour + path[endIndex:]
                    )
...
def limit_neighbours(G, neighbours, neighbourLimit, bottleneckFreeBER):
    sortedNeighbours = sorted(
        neighbours,
        key=lambda x: len(
            set(G.nodes[x]["features"]).intersection(set(bottleneckFreeBER))
        ),
        reverse=True,
    )
...
def find_detours(
    ...
    startNeighbours = list(
        set(nx.neighbors(G, detourStart))
        .difference(set(path))
        .difference(set(prefix))
        .difference(set(postfix))
    )
    endNeighbours = list(
        set(nx.neighbors(G, detourEnd))
        .difference(set(path))
        .difference(set(prefix))
        .difference(set(postfix))
    )

    startNeighbours = limit_neighbours(
        G, startNeighbours, neighbourLimit, bottleneckFreeBER
    )
    endNeighbours = limit_neighbours(
        G, endNeighbours, neighbourLimit, bottleneckFreeBER
    )
    ...
    for s in startNeighbours:
        for e in endNeighbours:
            if fulfillsStrictRequirements(G, PRO, s) and fulfillsStrictRequirements(
                G, PRO, e
            ):
                if (
                    s != e
                    and len(set([s, e]).intersection(set(path + prefix + postfix))) == 0
                ):
...
def globalBFS(G, PRO):
    if not fulfillsStrictRequirements(
        G, PRO, PRO.as_source
    ) or not fulfillsStrictRequirements(G, PRO, PRO.as_destination):
        return [], -1
