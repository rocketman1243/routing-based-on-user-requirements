...
limit_stage_path = "1_filter_stage"
comparison_experiment_path = "2_comparison_stage"
...
for graphType in graphTypes:
    ...
    if "comparison" in CHOSEN_PATH and not disableFullSearch:
        ...
        print(
            "Note: FINDING FULL PATH with slowpoke SMARTBFS. SO settle in cos this is going to take some time....."
        )
    ...
    for current_limits in limit_entries:
        if "comparison" in CHOSEN_PATH:
            outputFilePathHeuristic = (
                f"{CHOSEN_PATH}/results/{graphType}_{current_limits}.csv"
            )
            open(outputFilePathHeuristic, "w")
