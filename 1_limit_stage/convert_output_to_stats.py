...
def realisticScenarioBoxplots(runtimes, yLabel, xLabel, title):
    ...
    upperLimit = 50
    ax1.set_yticks(
        [i / 100 for i in range(0, upperLimit * 100 + 1, 5)],
        labels=[str(i * 10) for i in range(0, upperLimit * 100 + 1, 5)],
        color=c,
    )
    print(upperLimit)
    # ax1.set_yticks([i for i in range(0, upperLimit, 1000)], labels=[str(i) for i in range(0, upperLimit, 1000)], color="green")
...
realisticScenarioBoxplots(
    runtimesBoxplotDict,
    "runtime (milliseconds)",
    "[depthLimit, neighbourLimit]",
    f"Boxplots of runtimes for the realistic scenario experiment",
)
